...
const HeroBeats3: React.FC<any> = (props)=>{
  ...
  const imageHeroStyles = useMemo(() => ({
    filter: props.imageHero.styleData.filterImage.value.map((filter: any) => `${filter.name}(${filter.value})`).join(" "),
      ...
      <section className={`${styles.home} ${styles.section}`} style={sectionStyles} id="home">
        <div className={`${styles.home__container} ${styles.container} ${styles.grid}`}>
          <div className={styles.home__img__container}>
            <Image style={imageHeroStyles} height={750} width={400} src={props.imageHero.imageSrc} alt={props.imageHero.name} className={styles.home__img}></Image>
          </div>
          ...
          <div className={styles.home__data}>
            <div className={styles.home__header}>
              ...
              <h2 className={styles.home__subtitle} style={subtitleStyles}>{props.subtitleHero.content}</h2>
            </div>
            <div className={styles.home__footer}>
              ...
              <Link style={ctaButtonHeroStyles} href={props.ctaButtonHero.content.link} className={`${styles.button} ${styles.button__flex}`}>
